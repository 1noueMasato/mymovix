        ...
        <div class="card-body">
            <form action="{{ route('movies.update', $movie) }}" method="post">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="title" class="form-control" placeholder="" value="{{ old('title',$movie->title) }}"
                        aria-describedby="">
                    {{-- <small id="helpId" class="text-muted">Help text</small> --}}
                </div>
                <div class="form-group">
                    <label for="content">作品内容</label>
                    <textarea class="form-control auto-resize" name="content" id="content" cols="25" rows="20">{{ old('content',$movie->content) }}</textarea>
                </div>
                <div class="row">
                    <div class="form-group col-6">
                        <label for="">公開日</label>
                        <input type="date" name="screening_start_date" class="form-control" placeholder=""
                            value="{{ old('screening_start_date',$movie->screening_start_date) }}" aria-describedby="">
                    </div>
                    <div class="form-group col-6">
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" name="cast" id="cast" cols="25" rows="10">{{ old('cast',$movie->cast) }}</textarea>
                </div>
                <div class="form-group">
                    <label for="staff">製作者</label>
                    <textarea class="form-control" name="staff" id="staff" cols="25" rows="10">{{ old('staff',$movie->staff) }}</textarea>
                </div>
                {{-- <div class="form-group">
